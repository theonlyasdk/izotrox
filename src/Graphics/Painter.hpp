...
public:
    ...
    // Add radius to push_clip? Or new method?
    // User asked "take roundness ... into accounts when clipping".
    // Best to overloading push_clip or add push_rounded_clip.
    void push_clip(const IntRect& rect);
    void push_rounded_clip(const IntRect& rect, int radius);
    void pop_clip();
    
    void set_global_alpha(float alpha) { m_global_alpha = std::clamp(alpha, 0.0f, 1.0f); }
