...
void Painter::draw_pixel(IntPoint point, Color color)
{
    if (point.x < 0 || point.y < 0)
        return;
    if (point.x > Application::the().width() || point.y > Application::the().height())
        return;
    ...
    if (dx >= cr.x && dx < cr.right() && dy >= cr.y && dy < cr.bottom())
    {
        if (m_current_clip.radius > 0)
        {
            ...
            if (in_corner)
            {
                int dist_sq = (dx - cx + (dx < cx ? 0.5 : 0.5)) * (dx - cx + 0.5) +
                              (dy - cy + 0.5) * (dy - cy + 0.5); // Int approx? Using float logic from before is better?
